...
- The active product surface is run-native: tasks, config sets, immutable config versions, runs, run outputs, run chains, annotations, artifacts, eval plans, and schedules.
- The main frontend no longer presents chatrooms as the primary object.
- Legacy chatroom and experiment routes/files still exist for compatibility and historical tests, but they are not the target product contract.
- Prefer `/api/tasks`, `/api/config-sets`, `/api/runs`, `/api/run-chains`, `/api/annotations`, and `/api/artifacts` for new work.
- Use `requirements.md`, `plan.yaml`, and `data_structure_contracts.md` as the source of truth for the redesign.
